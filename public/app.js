...
function startProgressSimulation(steps, intervalMs = 700, pendingLabel = "等待模型响应") {
  clearProgressTimers();
  let index = 0;
...
function setStage(stage, description) {
  state.stage = stage;
  const labels = {
    input: ["等待输入", "上传产品图并填写需求，然后点击“分析产品”开始。", 0],
    analyzing: ["正在分析产品", description || "正在结合视觉模型与文本模型，生成完整设计方案。", 1],
    preview: ["确认图片方案", description || "检查设计规范和图片方案，确认后再生成最终图片。", 2],
    generating: ["正在生成图片", description || "先生成提示词，再渲染整组图片。", 3],
    complete: ["生成完成", description || "查看每张图片的最终结果。", 4]
  };
...
function resetResultsPlaceholder() {
  ...
  nodes.resultGrid.textContent = "生成完成后，这里会展示结果图片。";
}
...
function renderUploadGallery() {
  ...
  if (state.productImages.length === 0) {
    ...
    empty.innerHTML = `
      ...
      <span class="thumb-copy">点击上传产品图片</span>
    `;
    ...
    state.productImages.forEach((image, index) => {
      ...
      thumb.innerHTML = `
        ...
        <button type="button" class="thumb-delete" aria-label="删除图片">&times;</button>
      `;
      thumb.querySelector(".thumb-delete").addEventListener("click", (event) => {
        event.stopPropagation();
        state.productImages.splice(index, 1);
        renderUploadGallery();
        showDebug(`已删除 1 张产品图片，当前剩余 ${state.productImages.length} 张。`);
      });
...
async function handleAnalyze() {
  ...
  showDebug("已点击分析产品，正在准备请求 /api/analyze ...");
  ...
  startProgressSimulation(
    [
      { value: 8, label: "读取参考图片" },
      { value: 28, label: "提取产品主体信息" },
      { value: 52, label: "构建统一设计规范" },
      { value: 72, label: "规划图片布局" },
  ...
  try {
    ...
    showDebug(`分析完成，方案已生成，用时 ${((performance.now() - startedAt) / 1000).toFixed(1)} 秒。`);
...
async function handleGenerate() {
  ...
  const options = collectOptions();
  const startedAt = performance.now();
  showDebug("已点击生成图片，正在准备请求 /api/generate ...");
  nodes.analyzeBtn.disabled = true;
  nodes.generateBtn.disabled = true;
  setStage("generating");
  nodes.resultGrid.className = "result-grid empty-state";
  nodes.resultGrid.textContent = "正在生成提示词和图片，请稍候。";
  const count = Math.max(1, options.count);
  const generateSteps = [
    { value: 6, label: "准备图片方案" },
    { value: 14, label: "编写生成提示词" }
  ];
  for (let index = 0; index < count; index += 1) {
    generateSteps.push({
      value: Math.min(88, 24 + Math.round(((index + 1) / count) * 58)),
      label: `渲染图片 ${index + 1}/${count}`
    });
  }
  startProgressSimulation(generateSteps, 900, "等待图片模型响应");
  ...
  try {
    ...
    showDebug(`生成完成，共产出 ${data.images.length} 张图片，用时 ${((performance.now() - startedAt) / 1000).toFixed(1)} 秒。`);
    setStage("complete", `已成功生成 ${data.images.length} 张图片。`);
  } catch (error) {
    hideProgress();
...
nodes.fileInput.addEventListener("change", async (event) => {
  ...
  try {
    ...
    renderUploadGallery();
    showDebug(`已载入 ${newImages.length} 张产品图片，当前队列共 ${state.productImages.length} 张。`);
  } catch (error) {
...
nodes.pricingToggles.forEach((toggle) => {
  toggle.addEventListener("click", () => {
    ...
    showPricingFeedback(
      mode === "subscription"
        ? "这里是订阅展示区，当前按钮仅用于界面演示，不会触发真实扣费。"
        : "这里是积分购买展示区，当前按钮仅用于界面演示，不会触发真实支付。"
    );
  });
});
...
nodes.pricingActions.forEach((button) => {
  button.addEventListener("click", () => {
    const label = button.dataset.planLabel || "当前方案";
    showPricingFeedback(`已打开 ${label} 的演示入口。当前版本的套餐区仅做展示，不处理真实支付。`);
  });
});
